...
prompt_font_selection() {
    ...
    # Craft the formatted string dynamically
    for font in $fonts; do
        # Append font to the string in columns
        font_list="$font_list$(printf "%-35s" "$i - $font")"
        i=$((i + 1))

        # Add newline after every $cols fonts
        if [ $((i % cols)) -eq 0 ]; then
            font_list="$font_list\n"
        fi
    done

    # Add remaining fonts if the total count isn't a multiple of $cols
    if [ $((i % cols)) -ne 0 ]; then
        font_list="$font_list\n"
    fi
