    ...
    case "$PACKAGER" in
        pacman)
            ...
            if "$PACKAGER" -Qq pipewire-jack >/dev/null 2>&1; then
                "$ESCALATION_TOOL" "$PACKAGER" -S --needed lib32-pipewire-jack
            elif "$PACKAGER" -Qq jack2 >/dev/null 2>&1; then
                "$ESCALATION_TOOL" "$PACKAGER" -S --needed lib32-jack2
            fi

            $AUR_HELPER -S --needed $DEPENDENCIES $DISTRO_DEPS
            ;;
        apt-get | nala)
            "$ESCALATION_TOOL" dpkg --add-architecture i386
            "$ESCALATION_TOOL" "$PACKAGER" update
            
            "$ESCALATION_TOOL" "$PACKAGER" install $DEPENDENCIES
            ...
            "$ESCALATION_TOOL" "$PACKAGER" install $DISTRO_DEPS
            ;;
        dnf)
        ...
        eopkg)
            ...
            "$ESCALATION_TOOL" "$PACKAGER" install $DEPENDENCIES $DISTRO_DEPS
            ;;
        *)
