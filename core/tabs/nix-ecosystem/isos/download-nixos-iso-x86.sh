...
isUnstableMinimal() {
    [ "$1" = "nixos-unstable" ] && [ "$2" = "minimal" ]
}

printUnstableMinimalArt() {
    printf "%b" "${CYAN}"
    cat << 'EOF'
                         NIXOS
                        ⢀⣀⢸⡇⣀ 
...
EOF
    printf "%b\n" "${RC}"
    printf "%b\n" "${CYAN}  ❄️⋆❆°｡*̩̩͙❅｡ ISO ACQUIRED ｡❅*̩̩͙｡°❆⋆❄️${RC}"
    printf "%b\n" "${GREEN}  🦊🔥🔥🔥 SHA PASSED 🔥🔥🔥🦊${RC}"
}
...
resolveISO() {
    alias_url="https://channels.nixos.org/${1}/latest-nixos-${2}-x86_64-linux.iso"
    real_url=$(curl -sL -o /dev/null -w '%{url_effective}' "$alias_url")
    real_name=$(basename "$real_url")
    if [ "$real_name" = "latest-nixos-${2}-x86_64-linux.iso" ] || [ -z "$real_name" ]; then
...
downloadX86ISO() {
    ...
    printf "%b\n" "${CYAN}══════════════════════════════════════${RC}"
    printf "%b\n" "${CYAN}     NixOS ISO Downloader (x86_64)${RC}"
    printf "%b\n" "${CYAN}══════════════════════════════════════${RC}"
    printf "%b\n" ""
    ...
    printf "%b\n" "${YELLOW}Resolving latest ${ISO_TYPE} build for ${CHANNEL}...${RC}"
    ...
    if sha256sum -c "${ISO_NAME}.sha256" >/dev/null 2>&1; then
        ...
        if isUnstableMinimal "$CHANNEL" "$ISO_TYPE"; then
            printUnstableMinimalArt
        else
        ...
        printf "%b\n" ""
        printf "%b\n" "${GREEN}✓ Saved: ${DOWNLOAD_DIR}/${ISO_NAME}${RC}"
        printf "%b\n" ""
