...
try_flatpak_install() {
    appid="$1"
    checkFlatpak
    printf "%b\n" "${CYAN}Using Flatpak (fallback) for ${appid}...${RC}"
...
uninstall_app() {
    ...
    if [ -z "$appid" ] && [ -z "$pkgs" ]; then
        printf "%b\n" "${YELLOW}No uninstall targets found; nothing to remove.${RC}"
        return 0
    fi

    if [ -n "$appid" ]; then
        if uninstall_flatpak_if_installed "$appid"; then
            return 0
        fi
    fi
