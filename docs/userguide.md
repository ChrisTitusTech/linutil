...
- **Discord**: Discord is a versatile communication platform for gamers and communities, offering voice, video, and text chat features. Installed via Flatpak by default on most distros, with native packages used as a fallback where available.
...
- **Disk Usage**: GNOME Disk Usage Analyzer (Baobab) helps visualize disk usage and find large files.
